...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def save_model(self, sender, receiver, message):
            # 保存消息记录到数据库中
            from .models import Message
            return Message.objects.create(sender=sender, recipient=receiver, message=message)

        # @database_sync_to_async
    async def get_user(self, id=None, username=None):
            # 异步获取用户
            from One.models import LoginUser
            user = None
            if id:
                try:
    ...
    async def connect(self):
        ...
        if token:
            ...
            if last_login != timegm(user.last_login.utctimetuple()):
                raise DenyConnection("验证信息已过期")
        else:
            user = self.scope['user']
        if not user:
            raise DenyConnection("用户不存在")
        receiver_name = item.get('reciever')
    ...
    async def receive(self, text_data):
        ...
        if message:
            ...
            if ret:
                ...
                chat_record = await self.save_mode(self.user, self.receiver, message)
