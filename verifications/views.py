...
class SMSCode(View):
    ...
    def get(self, request, mobile):
        ...
        user = models.LoginUser.objects.filter(phone_numbers=mobile)
        if not user:
            response_data = {'code': 981, 'message': '手机号码未注册'}
        ...
        send_flag = redis_conn.get("send_flag_%s" % mobile)
        if send_flag:
            return http.JsonResponse({'code': 971, 'message': '发送短信过于频繁'})
        ...
        # 响应结果
        return http.JsonResponse({'code': 200, 'message': '发送短信成功', 'getCode': sms_code})
